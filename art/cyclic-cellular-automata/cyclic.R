...
setwd("/Users/thomas/projects/playground/art/cyclic-cellular-automata/")
...
# palette <- sample(clpalettes('random'), 1)[[1]] 
# colors <- palette %>% swatch %>% .[[1]]
display.brewer.all()
colors <- RColorBrewer::brewer.pal(name = "YlOrRd", n = 9)
colors <- RColorBrewer::brewer.pal(name = "YlGnBu", n = 9)
colors <- RColorBrewer::brewer.pal(name = "Blues", n = 9)
colors <- RColorBrewer::brewer.pal(name = "Spectral", n = 10)
# Do the plot
...
ggsave(
  paste0(format(Sys.time(), "%Y%m%d_%H%M%S_"), "art.png"),
  plot = last_plot(),
  device = NULL,
  path = NULL,
  scale = 1,
